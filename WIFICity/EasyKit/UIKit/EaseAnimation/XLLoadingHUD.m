...
+(XLLoadingHUD*)showIn:(UIView*)view{
    [self hideIn:view];
    XLLoadingHUD *hud = [[XLLoadingHUD alloc] initWithFrame:view.bounds];
    [hud start];
    [view addSubview:hud];
    return hud;
}
...
+(XLLoadingHUD *)hideIn:(UIView *)view{
    XLLoadingHUD *hud = nil;
    for (XLLoadingHUD *subView in view.subviews) {
        if ([subView isKindOfClass:[XLLoadingHUD class]]) {
            [subView hide];
