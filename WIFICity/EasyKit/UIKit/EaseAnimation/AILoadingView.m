...
- (UIBezierPath*)cycleBezierPathIndex:(NSInteger)index {
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.bounds.size.width * 0.5, self.bounds.size.height *0.5) radius:width * 0.5 startAngle:index * (M_PI* 2)/3  endAngle:index * (M_PI* 2)/3 + 2*M_PI * 4/3 clockwise:YES];
    return path;
}
...
- (void)loadingAnimation {
    ...
    strokeAniamtionGroup.repeatCount = 10000;
